...
func _ready():
	...
	for target in TARGETS:
		init_pos.append(target.position)
...
func _physics_process(delta):
	...
	for i in range(TARGETS.size()):
		TARGETS[i].rotation = rng.randf_range(-shake_strength, shake_strength) * 10e-4
		TARGETS[i].position = init_pos[i] + shake_offset
